...
def extract_next_path_points(path_points, pos, N):  # pos是car当前位置，先找到最近的index，然后返回其之后的N个点
    ...
    idx = find_closest_point(path_points, pos)
    next_path_points = np.zeros((2, N))
    for i in range(1, N + 1):
        next_path_points[:, i - 1] = path_points[:, idx + 6 * i]
...
def generate_pathplanner():
    ...
    # model.objective = obj
    # model.objective = lambda z: (400.0 * (z[2]) ** 2  # costs on deviating on the path in x-direction
    #                                 + 400.0 * (z[3]) ** 2  # costs on deviating on the path in y-direction
    #                                 + 200 * z[4] ** 2  # penalty on steering angle
    #                                 + 20 * (z[5] - desired_velocity) ** 2  # penalty on velocity
    #                                 + 0.1 * z[6] ** 2
    #                                 + 0.1 * z[0] ** 2  # penalty on input velocity of steering angle
    #                                 + 0.1 * z[1] ** 2)  # penalty on input longitudinal acceleration
    ...
    model.eq = lambda z: forcespro.nlp.integrate(Vehicle_dynamics.KS_casadi, z[2:7], z[0:2],
